...
- 사용자별 토스증권 연결 저장 기반은 구현됐다. Client ID와 Client Secret은 AES-256-GCM으로 암호화한 전용 테이블에만 저장하며, 암호화 키가 없는 환경에서는 연결 요청을 `503`으로 차단한다. 실제 토스 토큰 발급·계좌 검증·보유 종목 조회는 다음 단계다.
