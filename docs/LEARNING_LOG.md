...
1. Track A 손절 리서치에서 고정 비율 손절은 추가 검증 필요, ATR·추적 손절은 채택 비추천으로 판정됐다. 손절 없는 주문 초안 허용 여부와 `TradePlan.stopLossPrice` 필수 제약을 결정하기 전까지 자동 주문 초안은 만들지 않는다.
...
5. 현재 누적 변경을 기능 단위로 검토한 뒤 커밋한다. 커밋 전 백엔드 전체 테스트, 프론트엔드 lint/build, 주요 화면 수동 검증을 다시 실행한다.
