...
4. 위험 경고와 위험 한도 편집 화면은 `npm run lint`, `npm run build`, 수동 API 연동 확인까지 마쳤다. 다음 프론트엔드 학습에서는 기존 API 계약을 확인한 뒤 포트폴리오 평가 또는 전략 가이드 중 하나를 작은 화면 기능으로 연결한다.
...
현재 브랜치와 git status를 확인한 뒤, `frontend`에서 `npm run lint`, `npm run build`를 실행해 현재 변경을 먼저 확인한다. 위험 경고 목록은 `App`의 `map()`과 `RiskAlertItem`의 `alert` prop으로 분리돼 있고, 위험 한도는 조회값을 퍼센트 입력 폼에 표시해 클라이언트 검증 후 `PUT`으로 저장한다. 저장 성공·실패 메시지와 저장 후 위험 경고 재조회까지 구현됐다. 이 흐름을 짧게 회상한 뒤, 포트폴리오 평가 또는 전략 가이드 API 중 하나를 다음 작은 화면 기능으로 제안한다.
