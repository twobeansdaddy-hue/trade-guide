...
등록된 `TRACK_A` 프로필을 후보 유니버스로 사용하고, 포트폴리오의 보유 종목을 제외한 후보에 `BUY`/`WATCH` 판단을 제공한다. 현재 후보 API는 S&P 500 전체나 `TRACK_B`를 스크리닝하지 않는다. 다종목 조회는 후보별 외부 시세 API를 호출하며, 히스토리 캐시와 종목별 부분 실패 응답은 아직 없다.
...
1. 다종목 전략 가이드의 히스토리 캐시와 일부 종목 조회 실패 응답 형식을 설계한다.
2. `TradePlan`의 주문 가격·수량·유효 기간과 위험 정책을 전략 판단에서 분리해 모델링한다.
...
현재 브랜치와 git status를 확인한 뒤, 다종목 전략 가이드의 히스토리 캐시와 일부 종목 조회 실패 응답 형식을 설계부터 이어서 진행한다.
