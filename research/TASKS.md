...
- [ ] 착수 전 확인 필요: (a) 이 사용자가 실제로 국내 주식 전략 가이드까지 원하는지,
      (b) 국내 종목 실데이터(주봉 이력, 펀더멘털)를 무료로 확보할 수 있는 소스가
      있는지(Twelve Data는 US 전용, Yahoo Finance KR 지원 여부 미확인, 토스 자체
      API가 리서치용 대량 이력 조회를 지원하는지도 미확인).
- [ ] 데이터 확보 가능성 확인 후, Track B와 동일한 방식(사전 등록 → 워크포워드 →
      채택 게이트)으로 국내 종목 후보 검증 착수.
