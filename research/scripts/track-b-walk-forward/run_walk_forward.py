...
`docs/agent-tasks/track-b-walk-forward-data-feasibility.md`가 요구한 실행 파라미터를
아래에 사전 고정한다(결과를 보고 바꾸지 않음). `momentum_engine.py`의 계산 코어를 이용해
research/data/cache/track-b-walk-forward/universe_weekly.csv(고정 연구 유니버스 = DJIA
30종목, pointInTimeUniverse=false)에 대해 두 가지 사전 고정 비중첩 분할 체계(2분할/4분할)와
비용 가정(0/20/40/57bp, 왕복)으로 워크포워드를 실행하고, 동일가중 매수후보유 벤치마크와
비교한 원자료를 JSON으로 저장한다.

실행: python3 research/scripts/track-b-walk-forward/run_walk_forward.py

전략 규칙(track-b-strategy-catalogue.md / track-b-cross-sectional-momentum-validation.md
제안을 그대로 사용, 아직 미채택 파라미터임을 report에도 명시):
  - 형성기간: 52주 수익률에서 최근 4주 제외(48주)
  - 리밸런싱: 13주(분기)
  - 상위/유지 이원 기준: 상위 20% 신규 진입, 상위 40% 밖으로 밀려나기 전까지 유지
  - 동일가중 포트폴리오
"""
import json
import os
...
# 사전 고정 비중첩 분할 (실행 전 확정, 결과를 보고 바꾸지 않음)
PARTITIONS = {
    "2way": [
...
def run_all():
    ...
    output = {
        "generatedBy": "research/scripts/track-b-walk-forward/run_walk_forward.py",
        "universe": {
            "size": len(universe),
    ...
    for partition_name, windows in PARTITIONS.items():
        ...
        for w in windows:
            ...
            for bps in COST_BPS_SCENARIOS:
                ...
                # 벤치마크는 전략의 첫 리밸런싱(실제 투자 개시) 시점부터 시작해야 공정하다.
                # w_start부터 시작하면 전략은 첫 리밸런싱 전까지 무투자(현금) 구간이 생겨
                # 벤치마크만 그 구간의 시장 움직임을 먼저 반영하는 불공정한 비교가 된다.
                ...
                # 원자료(리밸런싱 이벤트) 별도 보관 - 0bp만 대표로 전체 저장(용량), 유상 시나리오는 요약만
                if bps == 0.0:
