...
Trade Guide는 투자 수익을 보장하지 않는다. 기본은 의사결정 지원이며, 주문 자동 실행은 사용자가 명시적으로 opt-in한 경우에 한해 검증된 전략 규칙에만 적용한다(2026-09-17 결정, `CLAUDE.md`). 이 문서는 리서치 결과를 전략 가이드와 사용자가 검토하는 예약 주문 계획으로 전환할 때 적용하는 기준이다.
...
| 상태 | 의미 | 현재 예시 |
| --- | --- | --- |
| 확정 요구사항 | 사용자가 구현 목표로 합의한 기능 또는 제약 | 미국 주식 대상, 보유 종목 가이드, 신규 후보 발굴, 장 시작 전 주문 계획 제시, 자동 주문은 opt-in 사용자에 한함(2026-09-17) |
...
## 자산 분류 프레임 재정의 (2026-09-17)

### 배경

기존 Track A(레버리지/고변동성)·Track B(일반 미국 주식)·Track C(국내 주식) 구분은 시장(미국/국내)과 상품 특성(레버리지/일반)을 하나의 축으로 묶어 놓은 것이었다. 2026-09-17 사용자와의 리서치 방향 재정의에서, 목표를 "SOXL 중심 Track A 튜닝"에서 "미국·국내 주식을 아우르는 시장 예측 엔진"으로 넓히기로 했다. 세부 목표는 다음 세 가지다.

- **(A) 자산군 일반화**: 특정 종목(SOXL) 중심이 아니라 미국·국내 주식 전반을 아우르는 분류·평가 체계.
- **(B) 매크로 기반 가이드 엔진**: 지수, 뉴스, 공시, 유가, 금, 금리, 인플레이션, VIX 등을 프리마켓/애프터마켓/본장 시점에 분석해 보유 포트폴리오에 지침을 주는 엔진.
- **(C) 가격 사다리 실행 가이드**: 보유·후보 종목에 대해 "$100에 1주 추가매수, $110에 부분매도, $90에 전량매도"처럼 구체적 가격대별 매수·부분매도·전량매도 지침. `research/STRATEGY_ENGINE_POLICY.md`의 기존 v2 보류 항목(전략 자동 손절, `quantityRatio` 자동 산출, `TradePlanGenerator`)을 다시 여는 것과 같은 방향이며, 그때 보류한 이유(고정비율·ATR·추적손절 전부 기각)를 이번엔 왜 다르게 볼 수 있는지부터 짚어야 한다.

세 목표 중 **(A) 자산군 일반화부터 착수**하기로 했다.

### 왜 시장(US/KR) 축이 아니라 데이터 계층인가

Track C(국내 주식에 Track A 규칙 이식)의 기각(`research/reports/track-c-kospi200-track-a-rule-validation.md`)이 "국내 시장이라서" 실패한 것인지, 애초에 적용한 방법론(레버리지 ETF용 추세 규칙)이 국내 일반 주식과 안 맞아서인지 구분되지 않았다. 또한 Track B 방법론(PEG/퀄리티 팩터)은 Finnhub 무료 펀더멘털 데이터가 국내 종목에 403으로 접근 불가(`research/reports/track-b-fundamental-data-provider-evaluation.md`)라 국내 종목엔 애초에 시도할 수 없다. 즉 시장 라벨은 실제 결정 요인(확보 가능한 데이터 계층, 자산의 변동성·decay 구조)의 대리 지표일 뿐이었다.

### 2축 분류 프레임

**축 1: 데이터 계층(Data Tier)** — 이 자산에 대해 실제로 어떤 데이터를 확보할 수 있는가.

- Tier 1 (가격만): 가격·거래량만 확보 가능. 국내 종목 대부분, 국내 레버리지 ETF(존재·데이터 확보 여부 확인 필요).
- Tier 2 (가격+펀더멘털): 미국 대형주 등 Finnhub 무료 티어로 재무 시계열 확보 가능.
- Tier 3 (가격+펀더멘털+매크로 연계 확인): 목표 (B) 매크로 엔진을 실제로 시도할 수 있는 자산군. 아직 어떤 자산도 이 계층으로 확인되지 않았다 — 매크로 데이터 제공자 조사가 선행 과제다.

**축 2: 변동성/구조 특성** — `AGENTS.md`가 예고한 베타·ATR 기준. decay가 있는 레버리지/고변동성 자산 vs 일반 주식으로 구분하며, 자동 분류 기준(베타·ATR 임계값)은 아직 검증 전이라 지금은 수동 등록(`AssetProfile`)을 유지한다.

### 기존 Track 결과의 재해석

| 기존 명칭 | 데이터 계층 | 변동성/구조 | 결과 |
| --- | --- | --- | --- |
| Track A (SOXL/TQQQ) | Tier 1 (가격만) | 레버리지/decay | 손절 4종 전부 기각, 진입 0~4주 규칙만 채택 |
| Track B (S&P100) | Tier 2 (가격+펀더멘털) | 일반 | 9개 후보 전부 기각, 스크리닝만 유지 |
| Track C (KOSPI200) | Tier 1 (가격만) | 일반 | Track A 규칙 이식 기각 — **일반 주식에 레버리지 방법론을 쓴 것이 원인일 수 있음**, 국내 시장 자체의 결함으로 단정할 근거는 아직 없음 |

국내 종목은 Tier 1(펀더멘털 데이터 없음)이므로, Track C(국내 일반주)를 Track B 방법론으로 재시도하려면 펀더멘털 기반 후보(PEG/퀄리티)는 애초에 이식 불가하고 가격 기반 후보(모멘텀 등)만 시도 가능하다는 제약이 이번 재정의로 명확해졌다.

### 다음 단계 (자산군 일반화, 목표 A)

1. 국내 레버리지 ETF(예: KODEX 레버리지, TIGER 200선물레버리지 — 정확한 종목·데이터 확보 가능성은 확인 필요) 존재를 확인한다. 존재하면 "Tier 1 × 레버리지" 자산으로 Track A 방법론(가격 기반 추세·타이밍) 재시도 후보가 된다.
2. 현재 관리 중인 자산(SOXL, TQQQ, S&P100 대표 종목, KOSPI200 종목)이 Tier 1/2/3 중 어디에 속하는지 실제 데이터 제공자로 확인해 매핑표를 만든다.
3. 매크로 데이터(지수, 뉴스, 공시, 유가, 금, 금리, 인플레이션, VIX) 제공자 조사 — 어떤 자산을 Tier 3로 올릴 수 있는지 확인한다(목표 B의 선행 조건).
4. 목표 (B) 매크로 엔진, (C) 가격 사다리 가이드는 이 매핑이 끝난 뒤 순서대로 착수한다.
...
- v1 전략 판단 자체는 주문을 실행하지 않는다. 결과는 사용자가 검토하는 의사결정 지원 정보이며, opt-in 자동 주문은 별도 동의·킬스위치·전략 화이트리스트 경로(`BrokerOrderExecutionGrant`/`BrokerOrderExecutionRun`)에서만 다룬다.
