...
export default function HoldingsPage() {
    ...
    return <><header className="page-header"><p className="eyebrow">PORTFOLIO {selectedPortfolioId ?? "-"}</p><h1>보유 종목</h1><p>평가 API가 제공하는 현재 평가 기준의 보유 현황입니다.</p></header>{error ? <p className="status-message error" role="alert">{error}</p> : valuation ? <HoldingValuationList holdings={valuation.holdingValuations}/> : <p className="status-message" aria-live="polite">보유 종목을 불러오는 중입니다.</p>}</>;
}
