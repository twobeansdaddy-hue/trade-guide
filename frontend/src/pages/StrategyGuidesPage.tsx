...
export default function StrategyGuidesPage() {
    ...
    useEffect(() => { if (selectedPortfolioId === null) return; void getPortfolioStrategyGuides(memberId, selectedPortfolioId).then((data) => setHoldings({data, error: null})).catch((reason: unknown) => setHoldings({data: null, error: reason instanceof Error ? reason.message : "보유 종목 가이드를 불러오지 못했습니다."})); void getCandidateStrategyGuides(memberId, selectedPortfolioId).then((data) => setCandidates({data, error: null})).catch((reason: unknown) => setCandidates({data: null, error: reason instanceof Error ? reason.message : "후보 가이드를 불러오지 못했습니다."})); }, [memberId, selectedPortfolioId]);
