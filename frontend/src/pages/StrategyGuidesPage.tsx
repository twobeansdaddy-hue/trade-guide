...
function StrategyGuidesContent({memberId, portfolioId}: {memberId: number; portfolioId: number}) {
    ...
    return <><header className="page-header"><p className="eyebrow">TRACK A</p><h1>전략 가이드</h1><p>가이드는 자동 주문이 아닌 최종 판단 전 검토 정보입니다.</p>{dataAsOfLabel ? <p className="data-as-of">시세 데이터 기준일 {dataAsOfLabel}</p> : null}</header><section className="content-section"><div className="section-heading"><div><p className="section-label">HELD ASSETS</p><h2>보유 종목 가이드</h2></div></div>{render(holdingsResource, "전략 가이드를 표시할 보유 종목이 없습니다.", "보유 종목 가이드 다시 시도")}</section><section className="content-section"><div className="section-heading"><div><p className="section-label">CANDIDATES</p><h2>후보 가이드</h2></div></div>{render(candidatesResource, "현재 등록된 후보 가이드가 없습니다.", "후보 가이드 다시 시도")}</section></>;
}
