...
function DashboardContent({memberId, portfolioId}: {memberId: number; portfolioId: number}) {
    ...
    const alertsRateLimitCooldown = getApiRetryAfterSeconds(alertsResource.error);
    ...
    return <>
        ...
        {valuationResource.isLoading && !valuationResource.data ? (
            <p className="status-message valuation-status" aria-live="polite">포트폴리오 평가를 불러오는 중입니다.</p>
            ...
            <RequestError className="valuation-error" message={valuationResource.error.message} onRetry={valuationResource.refresh} retryLabel="평가 다시 시도"/>
        ...
        <section className="content-section risk-section">
            ...
            {alertsResource.isLoading ? (
                <p className="status-message" aria-live="polite">위험 경고를 불러오는 중입니다.</p>
            ) : isRiskPolicyMissing ? (
                <div className="empty-state empty-state-action"><p>위험 한도를 설정하면 종목별 비중 초과를 확인할 수 있습니다.</p><Link className="quiet-action" to="/settings">위험 한도 설정</Link></div>
            ) : isAlertsRateLimit ? (
                <MarketDataRateLimitNotice
                    message={alertsResource.error?.message}
                    onRetry={alertsResource.refresh}
                    retryLabel="위험 경고 다시 시도"
                    cooldownSeconds={alertsRateLimitCooldown}
                />
            ) : alertsResource.error ? (
                <RequestError message={alertsResource.error.message} onRetry={alertsResource.refresh} retryLabel="위험 경고 다시 시도"/>
            ) : alertsResource.data && alertsResource.data.length > 0 ? (
                <ul className="risk-alert-list">{alertsResource.data.map((alert) => <RiskAlertItem key={`${alert.market}-${alert.ticker}`} alert={alert}/>)}</ul>
            ) : (
                <p className="empty-state">현재 위험 한도를 넘는 보유 종목이 없습니다.</p>
            )}
        </section>
    </>;
}
