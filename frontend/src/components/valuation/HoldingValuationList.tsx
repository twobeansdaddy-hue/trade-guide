    ...
    return (
        <>
            <div className="holding-table-wrap">
                <table className="holding-table">
                    ...
                    <tbody>{holdings.map((holding) => <tr key={`${holding.market}-${holding.ticker}`}>
                        <th scope="row"><strong>{holding.ticker}</strong><span>{holding.market}</span></th>
            ...
            <ul className="holding-cards">{holdings.map((holding) => <li key={`${holding.market}-${holding.ticker}`}>
                <div className="holding-card-heading"><div><strong>{holding.ticker}</strong><span>{holding.market}</span></div><strong className={getProfitLossClassName(holding.returnRate)}>{formatPercent(holding.returnRate)}</strong></div>
